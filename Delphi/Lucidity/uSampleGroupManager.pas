...
type
  ...
  public
    ...
    function NewSampleGroup:IKeyGroup; overload;
    function NewSampleGroup(aName : string):IKeyGroup; overload;
    function FindSampleGroup(aName : string):IKeyGroup; overload;
    procedure DeleteSampleGroup(aName : string);

    procedure SetFocus(aName : string);

    // I'm not yet 100% convinced adding the 'FocusGroup' functionality into
    // TSampleGroupManager is a good idea.
  ...
  IKeyGroupsInfo = interface
    ...
    function GetSampleGroup(Index: integer): IKeyGroup;
    function GetSampleGroupCount: integer;
    function GetFocusedGroup : IKeyGroup;
  end;
  ...
  public
    ...
    property SampleGroupCount : integer read GetSampleGroupCount;
    property SampleGroups[Index : integer] : IKeyGroup read GetSampleGroup;
...
begin
  ...
  InitReference := TKeyGroup.Create(Voices, VoiceController, GlobalModPoints, Globals);
...
function TKeyGroupManager.NewSampleGroup(aName: string): IKeyGroup;
var
  sg : IKeyGroup;
begin
  ...
  try
    ...
    sg := TKeyGroup.Create(Voices, VoiceController, GlobalModPoints, Globals);
    ...
    //TODO: NOTE: Temporary assignment while getting focus handling set up.
    fFocusedGroup := sg;
  finally
    ListLock.Release;
  end;
...
function TKeyGroupManager.NewSampleGroup: IKeyGroup;
var
  sg : IKeyGroup;
begin
  ListLock.Acquire;
...
procedure TKeyGroupManager.DeleteSampleGroup(aName: string);
var
  c1: Integer;
  sg : IKeyGroup;
begin
  ...
  try
    ...
    begin
      sg := (fList[c1] as IKeyGroup);
      if sg.GetName = aName then fList.Remove(sg);
      sg := nil;
    end;
  finally
    ListLock.Release;
  ...
  if fList.Count = 0 then NewSampleGroup;
  if FocusedGroup.GetName = aName then fFocusedGroup := (fList[0] as IKeyGroup);
end;
...
procedure TKeyGroupManager.SetFocus(aName: string);
var
  sg : IKeyGroup;
begin
  sg := SampleGroup(aName);
  if assigned(sg) then fFocusedGroup := sg;
end;
...
begin
  ...
  self.fFocusedGroup := Source.FocusedGroup;
end;

function TKeyGroupsInfo.GetFocusedGroup: IKeyGroup;
begin
  result := fFocusedGroup;
end;

function TKeyGroupsInfo.GetSampleGroup(Index: integer): IKeyGroup;
begin
  result := GroupsList[Index] as IKeyGroup;
end;

function TKeyGroupsInfo.GetSampleGroupCount: integer;
begin
