...
type
  ...
  public
    ...
    // Call procedure LoadFromFile to load an XML document from the filename
    // specified. See Create for an example. The LoadFromFile_Safe function
    // will return false if it can not load the file due to non-wellformed XML.
    function LoadFromFile_Safe(const AFileName: string):boolean; virtual; 
...
function TNativeXml.LoadFromFile_Safe(const AFileName: string): boolean;
begin
  try
    ParserWarnings := false;
    LoadFromFile(AFileName);
  except
    // EFilerError is raised when the document is not well formed. Catch the error and return false.
    on EFilerError do
    begin
      result := false;
      exit; //==============================>>
