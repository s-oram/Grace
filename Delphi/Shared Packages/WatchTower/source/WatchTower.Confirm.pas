...
//  == CHANGE HISTORY ==
//  14th September 2015 [Shannon Oram]
//  - Renamed unit to WatchTower.Assert.pas for inclusion into WatchTower.
//  - Remove Assert.Pass() method.
//  - Renamed Assert to Confirm to avoid conflict with inbuilt Assert() method.

unit WatchTower.Confirm;
...
{$ENDIF}
    ...
    class property OnConfirm: TProc read fOnConfirm write fOnConfirm;
...
class procedure Confirm.AreEqual(const expected, actual, tolerance: Extended; const message: string);
begin
  DoAssert;
...
class procedure Confirm.AreEqual(const expected, actual: TClass; const message: string);
var
  msg : string;
begin
  DoAssert;
...
{$IFNDEF DELPHI_XE_DOWN}
    //Delphi 2010 and XE compiler bug breaks this
class procedure Confirm.AreEqual<T>(const expected, actual: T; const message: string);
var
...
class function Confirm.AddLineBreak(const msg: string): string;
begin
  if msg <> '' then
...
class procedure Confirm.AreEqual(const expected, actual: Integer; const message: string);
begin
  DoAssert;
  if expected <> actual then
...
class procedure Confirm.AreEqual(const expected, actual: boolean; const message: string);
begin
  DoAssert;
  if expected <> actual then
...
class procedure Confirm.AreEqual(const expected, actual, tolerance: Double; const message: string);
begin
  DoAssert;
  if not Math.SameValue(expected,actual,tolerance) then
    FailFmt('Expected %g but got %g %s' ,[expected,actual,message], ReturnAddress);
...
class procedure Confirm.AreEqual(const expected, actual: Double; const message: string);
var
  tolerance : Double;
begin
  tolerance := 0;
...
class procedure Confirm.AreEqual(const expected, actual: Extended; const message: string);
var
  tolerance : Extended;
begin
  tolerance := 0;
...
class procedure Confirm.AreEqualMemory(const expected : Pointer; const actual : Pointer; const size : Cardinal; const message : string);
begin
  DoAssert;
...
class procedure Confirm.AreNotEqual(const expected, actual, tolerance: Extended; const message: string);
begin
  DoAssert;
...
class procedure Confirm.AreNotEqual(const expected, actual: string;const ignoreCase: boolean; const message: string);
...
class procedure Confirm.AreNotEqual(const expected, actual: TClass; const message: string);
var
  msg : string;
begin
  DoAssert;
...
{$IFNDEF DELPHI_XE_DOWN}
//Delphi 2010 and XE compiler bug breaks this
class procedure Confirm.AreNotEqual<T>(const expected, actual: T; const message: string);
var
  comparer : IComparer<T>;
  expectedValue, actualValue : TValue;
begin
...
class procedure Confirm.AreNotEqual(const expected, actual: Integer; const message: string);
begin
  DoAssert;
  if expected = actual then
...
class procedure Confirm.AreNotEqual(const expected, actual: Extended; const message: string);
var
  tolerance : Extended;
begin
  tolerance := 0;
  Confirm.AreNotEqual(expected,actual,tolerance,message);
end;

class procedure Confirm.AreNotEqual(const expected, actual: Double; const message: string);
var
  tolerance : double;
begin
  tolerance := 0;
...
class procedure Confirm.AreNotEqual(const expected, actual, tolerance: double; const message: string);
begin
  DoAssert;
  if Math.SameValue(expected, actual, tolerance) then
    FailFmt('%g equals actual %g %s' ,[expected,actual,message], ReturnAddress);
...
class procedure Confirm.AreNotEqualMemory(const expected, actual: Pointer; const size: Cardinal; const message: string);
begin
  DoAssert;
...
class procedure Confirm.AreNotSame(const expected, actual: TObject; const message: string);
begin
  DoAssert;
...
class procedure Confirm.AreNotSame(const expected, actual: IInterface; const message: string);
begin
  DoAssert;
...
class procedure Confirm.AreSame(const expected, actual: IInterface; const message: string);
begin
  DoAssert;
  if (expected as IInterface) <> (actual as IInterface) then
...
class procedure Confirm.AreSame(const expected, actual: TObject; const message: string);
begin
  DoAssert;
...
{$IFDEF DELPHI_XE_UP}
//Delphi 2010 compiler bug breaks this
class procedure Confirm.Contains<T>(const list: IEnumerable<T>; const value: T; const message: string);
var
  o : T;
  comparer : IComparer<T>;
begin
...
class procedure Confirm.Contains<T>(const arr : array of T; const value : T; const message : string = '');
var
  o : T;
  comparer : IComparer<T>;
begin
...
{$IFDEF DELPHI_XE_UP}
//Delphi 2010 compiler bug breaks this
class procedure Confirm.DoesNotContain<T>(const list: IEnumerable<T>; const value: T; const message: string);
var
  o : T;
  comparer : IComparer<T>;
begin
...
class procedure Confirm.DoesNotContain<T>(const arr : array of T; const value : T; const message : string = '');
var
  o : T;
  comparer : IComparer<T>;
begin
...
class procedure Confirm.Fail(const message : string; const errorAddrs : pointer);
begin
...
class procedure Confirm.FailFmt(const message: string; const args: array of const; const errorAddrs: pointer);
begin
  Fail(Format(message, args), errorAddrs);
end;

class function Confirm.Implements<T>(value: IInterface; const message: string) : T;
begin
  DoAssert;
...
class procedure Confirm.InheritsFrom(const descendant, parent: TClass; const message: string);
var
  msg : string;
begin
  DoAssert;
...
class procedure Confirm.IsEmpty(const value: IInterfaceList; const message: string);
begin
  DoAssert;
...
class procedure Confirm.IsEmpty(const value: TList; const message: string);
begin
  DoAssert;
  if value.Count > 0 then
    FailFmt('List is Not empty. %s',[message], ReturnAddress);
...
class procedure Confirm.IsEmpty(const value, message: string);
begin
  DoAssert;
...
class procedure Confirm.IsEmpty(const value: Variant; const message: string);
begin
  DoAssert;
...
class procedure Confirm.IsEmpty(const value: TStrings; const message: string);
begin
  DoAssert;
  if value.Count > 0 then
    FailFmt('List is Not empty. %s',[message], ReturnAddress);
...
{$IFDEF DELPHI_XE_UP}
//Delphi 2010 compiler bug breaks this
class procedure Confirm.IsEmpty<T>(const value: IEnumerable<T>; const message: string);
var
  o : T;
  count : integer;
begin
...
class procedure Confirm.IsFalse(const condition: boolean; const message: string);
begin
  DoAssert;
  if condition then
...
class procedure Confirm.IsNotEmpty(const value: TList; const message: string);
begin
  DoAssert;
  if value.Count = 0 then
...
class procedure Confirm.IsNotEmpty(const value: IInterfaceList; const message: string);
begin
  DoAssert;
  if value.Count = 0 then
   FailFmt('List is Empty. %s',[message], ReturnAddress);
...
class procedure Confirm.IsNotEmpty(const value: TStrings; const message: string);
begin
  DoAssert;
  if value.Count = 0 then
   FailFmt('List is Empty. %s',[message], ReturnAddress);
...
class procedure Confirm.IsNotEmpty(const value, message: string);
begin
  DoAssert;
  if value = '' then
...
class procedure Confirm.IsNotEmpty(const value: Variant; const message: string);
begin
  DoAssert;
...
{$IFDEF DELPHI_XE_UP}
//Delphi 2010 compiler bug breaks this
class procedure Confirm.IsNotEmpty<T>(const value: IEnumerable<T>; const message: string);
var
  x : T;
  count : integer;
begin
...
class procedure Confirm.IsNotNull(const condition: IInterface; const message: string);
begin
  DoAssert;
...
class procedure Confirm.IsNotNull(const condition: Pointer; const message: string);
begin
  DoAssert;
  if condition = nil then
...
class procedure Confirm.IsNotNull(const condition: TObject; const message: string);
begin
  DoAssert;
  if condition = nil then
...
class procedure Confirm.IsNotNull(const condition: Variant; const message: string);
begin
  DoAssert;
...
class procedure Confirm.IsNull(const condition: Variant; const message: string);
begin
  DoAssert;
...
class procedure Confirm.IsNull(const condition: IInterface; const message: string);
begin
  DoAssert;
  if condition <> nil then
...
class procedure Confirm.IsNull(const condition: TObject; const message: string);
begin
  DoAssert;
  if condition <> nil then
...
class procedure Confirm.IsNull(const condition: Pointer; const message: string);
begin
  DoAssert;
  if condition <> nil then
...
class procedure Confirm.IsTrue(const condition: boolean;const message : string);
begin
  DoAssert;
...
class procedure Confirm.NotImplemented;
begin
  Confirm.Fail('Not implemented!');
end;

{$IFDEF DELPHI_XE_UP}
//Delphi 2010 compiler bug breaks this
class procedure Confirm.IsType<T>(const value: T; const message : string);
var
  val : TValue;
begin
  DoAssert;
...
class procedure Confirm.WillNotRaise(const AMethod: TTestMethod; const exceptionClass: ExceptClass; const msg: string);
begin
  Confirm.WillNotRaise(
    procedure
    begin
      AMethod;
    end,
...
class procedure Confirm.WillNotRaiseAny(const AMethod: TTestLocalMethod;  const msg: string);
begin
  DoAssert;
  try
    AMethod;
...
class procedure Confirm.WillNotRaiseAny(const AMethod: TTestMethod;const msg: string);
begin
  Confirm.WillNotRaiseAny(
    procedure
    begin
      AMethod;
    end,
...
class procedure Confirm.WillNotRaiseDescendant(const AMethod: TTestLocalMethod;  const exceptionClass: ExceptClass; const msg: string);
begin
  DoAssert;
  try
    AMethod;
...
class procedure Confirm.WillNotRaiseDescendant(const AMethod: TTestMethod;  const exceptionClass: ExceptClass; const msg: string);
begin
  Confirm.WillNotRaise(
    procedure
    begin
...
class procedure Confirm.WillRaise(const AMethod : TTestLocalMethod; const exceptionClass : ExceptClass; const msg : string);
begin
  DoAssert;
  try
    AMethod;
...
class procedure Confirm.WillNotRaise(const AMethod : TTestLocalMethod; const exceptionClass : ExceptClass; const msg : string);
begin
  DoAssert;
  try
    AMethod;
...
class procedure Confirm.WillRaise(const AMethod: TTestMethod; const exceptionClass: ExceptClass; const msg: string);
begin
  Confirm.WillRaise(
    procedure
    begin
...
class procedure Confirm.WillRaiseAny(const AMethod: TTestLocalMethod; const msg: string);
begin
  DoAssert;
  try
    AMethod;
...
class procedure Confirm.WillRaiseAny(const AMethod: TTestMethod; const msg: string);
begin
  Confirm.WillRaiseAny(
    procedure
    begin
      AMethod;
    end,
...
class procedure Confirm.WillRaiseDescendant(const AMethod: TTestLocalMethod;  const exceptionClass: ExceptClass; const msg: string);
begin
  DoAssert;
  try
    AMethod;
...
class procedure Confirm.WillRaiseDescendant(const AMethod: TTestMethod; const exceptionClass: ExceptClass; const msg: string);
begin
  Confirm.WillRaiseDescendant(
    procedure
    begin
      AMethod;
    end,
...
class procedure Confirm.WillRaiseWithMessage(const AMethod: TTestLocalMethod; const exceptionClass: ExceptClass;const exceptionMsg, msg: string);
begin
  DoAssert;
  try
    AMethod;
...
class procedure Confirm.AreEqual(const expected : string; const actual : string; const message : string);
begin
  Confirm.AreEqual(expected, actual, true, message);
end;

class procedure Confirm.AreEqual(const expected, actual : string;  const ignoreCase : boolean; const message: string);
begin
  DoAssert;
  if ignoreCase then
  begin
...
class procedure Confirm.CheckExceptionClass(E: Exception; const exceptionClass: ExceptClass);
begin
  DoAssert;
  if exceptionClass = nil then
    Exit;
...
class procedure Confirm.CheckExceptionClassDescendant(E: Exception;
...
class procedure Confirm.Contains(const theString : string; const subString : string; const ignoreCase : boolean; const message : string);
begin
  DoAssert;
  if ignoreCase then
  begin
...
class procedure Confirm.DoAssert;
begin
  if Assigned(fOnConfirm) then
    fOnConfirm();
end;

class procedure Confirm.EndsWith(const subString : string; const theString : string; const ignoreCase : boolean; const message : string);
begin
  DoAssert;
  if ignoreCase then
  begin
...
{$IFDEF SUPPORTS_REGEX}
class procedure Confirm.IsMatch(const regexPattern, theString, message: string);
begin
  DoAssert;
...
class procedure Confirm.StartsWith(const subString : string; const theString : string; const ignoreCase : boolean; const message: string);
begin
  DoAssert;
