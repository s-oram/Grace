...
type
    ...
    protected
      ...
      // HandlePluginException() provides a chance for plugins to handle any exceptions.
      function HandlePluginException:boolean; virtual;

      function GetNumParams: VstInt32;
      function GetNumPrograms: VstInt32;
    ...
    public
      ...
      // Processing
      procedure ProcessReplacing(Inputs, Outputs: PPSingle; SampleFrames: VstInt32); virtual;    // Process 32 bit (single precision) floats (always in a resume state)
      ...
      property NumPrograms  : VstInt32  read GetNumPrograms;
      property NumParams    : VstInt32  read GetNumParams;

      property NumInputs    : VstInt32 read GetNumInputs;
      property NumOutputs   : VstInt32 read GetNumOutputs;
    end;
...
var
  obj : AudioEffect;
  ReturnValue : VstIntPtr;
begin
  obj := e^.vObject;

  try
    ReturnValue := obj.Dispatcher(opCode, index, value, ptr, opt);
  except
    on ExObj : Exception do
    begin
      ReturnValue := 0;

      if obj.HandlePluginException = false
        then raise;
    end;
  end;
...
begin
  try
    ...
    begin
      result := 0;
      if AudioEffect(e^.vObject).HandlePluginException = false
        then raise;
    end;
  end;
end;
...
function AudioEffect.GetNumInputs: VstInt32;
begin
  result := FEffect.numInputs;
end;

function AudioEffect.GetNumOutputs: VstInt32;
begin
  result := FEffect.numOutputs;
end;

function AudioEffect.GetNumParams: VstInt32;
begin
  result := FEffect.numParams;
end;

function AudioEffect.GetNumPrograms: VstInt32;
begin
  result := FEffect.numPrograms;
end;
...
function AudioEffect.HandlePluginException:boolean;
begin
  result := false;
end;
...
function AudioEffect.GetBlockSize: VstInt32;
begin
  result := self.FBlockSize;
end;
...
procedure AudioEffect.ProcessReplacing(Inputs, Outputs: PPSingle; SampleFrames: VstInt32);
begin
  // do nothing
end;
